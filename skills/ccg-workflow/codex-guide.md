...
除了标准的 Codex 审核，CCG 还提供企业级审核模式（`/codex-code-review-enterprise`）。

### 与标准审核的区别

| 维度 | 标准审核 | 企业级审核 |
|------|---------|-----------|
| 触发方式 | `mcp__ccg__codex` | `/codex-code-review-enterprise` |
| 审核深度 | 基础质量检查 | 企业级质量门禁 |
| 审核维度 | 3 个维度 | 4 个维度（增加演进安全） |
| 输出格式 | 简单结论 | Blocking/Non-blocking/Nit 分级 |
| 适用场景 | 日常开发 | PR 合入、上线前 |
...
详细使用方法请参考 `/codex-code-review-enterprise` 技能文档。
