...
description: 测试失败多层级修复（4层策略：Coder → Codex → Claude → Gemini）
category: Testing
...
> **用途**：当单层 Coder 修复失败时，使用多层级策略提高修复成功率

## 核心思想

充分利用 CCG 系统中所有 AI 的能力，通过 4 层修复策略提高测试失败的自动修复成功率。
...
**角色**：Coder（GLM-4.7 或其他配置的模型）
**尝试次数**：最多 2 次
**预期成功率**：60-70%

### 执行步骤

**步骤 1：使用 acemcp 搜索相关代码**
```json
{
...
**修复尝试**：
- 第 1 层（Coder）：2 次尝试，失败
- 第 2 层（Codex + Coder）：1 次尝试，失败
- 第 3 层（Claude）：1 次尝试，失败
- 第 4 层（Gemini）：1 次尝试，失败
